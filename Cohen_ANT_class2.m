...
for chani=1:datadim(1)
    for chanj=1:datadim(1)
        ...
        % mean-center data
        subi = data(chani,:) - mean( data(chani,:) );
        subj = data(chanj,:) - mean( data(chanj,:) );
        
        % compute covariance as dot product divided by N-1
        covmat1(chani,chanj) = sum( subi.*subj ) / (datadim(2)-1);
    end % end chanj
end % end chani
...
covmat2 = dataM*dataM' / (datadim(2)-1);
...
%topoplotIndieOctave(lf.GainN(:,diploc), EEG.chanlocs,'numcontour',0,'electrodes','numbers','shading','interp'); % for OCtave users
...
plot(EEG.times,dipole_data(diploc,:)/norm(dipole_data(diploc,:)),'linewidth',4)
plot(EEG.times,EEG.data(31,:)/norm(EEG.data(31,:)),'linewidth',2)
...
subplot(212), hold on
plot(EEG.times,.3+dipl_ts,'linewidth',2)
...
plot(timevec,bsxfun(@plus,mean(csd,3),(1:16)'*500),'linewidth',2)
...
timewin_1 = [ .55 .7];   % time in seconds
timewin_2 = [-.20  0]; % time in seconds
...
for triali=1:size(csd,3)
    ...
    %%% S
    % extract data
    tmp = csd(:,tidxS(1):tidxS(2),triali);
    % mean-center
    ...
    %%% repeat for R
    % extract data
    tmp = csd(:,tidxR(1):tidxR(2),triali);
    % mean-center
    tmp = bsxfun(@minus,tmp,mean(tmp,2));
    % add to covariance matrix
...
plot(timevec,compERP, timevec,chanERP,'linewidth',2)
